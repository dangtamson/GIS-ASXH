...
async function createAccount(account: AccountInsertType): Promise<AccountSelectType> {
  ...
  const [inserted]: AccountSelectType[] = await db.insert(accounts).values(account).onConflictDoNothing({ target: accounts.email }).returning();
  const result = inserted || (await db.select().from(accounts).where(eq(accounts.email, account.email)).limit(1))[0];
...
async function createWorkspace(name: string, description: string, accountId: string): Promise<WorkspaceSelectType> {
  const [existingWorkspace] = await db
    .select()
    .from(workspaces)
    .where(and(eq(workspaces.name, name), eq(workspaces.accountId, accountId)))
    .limit(1);

  if (existingWorkspace) {
    logger.info(`Workspace already exists: ${existingWorkspace.name} (${existingWorkspace.uuid})`);
...
async function createProfile(workspaceId: string, accountId: string, profileName: string): Promise<ProfileSelectType> {
  const [existingProfile] = await db
    .select()
    .from(profiles)
    .where(and(eq(profiles.workspaceId, workspaceId), eq(profiles.accountId, accountId)))
    .limit(1);

  if (existingProfile) {
    logger.info(`Profile already exists: ${existingProfile.name} in workspace ${workspaceId}`);
...
async function createMembership(
  ...
  if (existingMembership) {
    logger.info(`Membership already exists for account ${accountId} in workspace ${workspaceId}`);
    return existingMembership;
  }

  const [roleRecord] = await db.select({ id: roles.id }).from(roles).where(eq(roles.code, role)).limit(1);
  if (!roleRecord) {
    throw new Error(`Unable to find role id for role: ${role}`);
  }

  const [membership] = await db
    .insert(workspaceMemberships)
    .values({ roleId: roleRecord.id, workspaceId, accountId })
    .returning();
