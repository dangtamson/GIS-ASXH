...
const reportDetailQuerySchema = reportQuerySchema.extend({
  document_uuid: z.string().trim().optional(),
...
function buildDocumentTaskScopeQuery(input: {
  workspaceId: string;
  organizationIds: string[];
  fromDate?: string;
  toDate?: string;
  ...
  const documentFilter = documentUuid ? sql`and d.uuid = ${documentUuid}` : sql``;
...
export const reportTaskByDocument = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const { rows } = await db.execute(sql`
    ...
    select
      ...
      max(ft.document_number) as document_number,
      max(ft.document_title) as title,
    ...
    group by ft.document_uuid
    order by max(ft.document_number) nulls last, max(ft.document_title)
  `);
  ...
  const response = apiResponse.success(
    ...
    "Report task by document retrieved successfully"
  );

  res.status(response.code).send(response);
});

export const reportTaskByDocumentDetail = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const workspaceId = req.workspaceId?.trim();
  if (!workspaceId) {
    const response = apiResponse.error(HttpErrors.MissingParameter("x-workspace-id header"));
    res.status(response.code).send(response);
  ...
  const { fromDate, toDate, organization_ids, field, document_type_id, document_uuid, metric, page, limit } =
    parsed.data;
  const fieldList = parseList(field);
  const docTypeList = parseList(document_type_id);
  const organizationIds = await resolveOrganizationIds(
  ...
  const response = apiResponse.success(
    HttpStatusCode.OK,
    {
      ...
      filters: {
        ...
        field: fieldList.length ? fieldList : null,
        documentTypeId: docTypeList.length ? docTypeList : null,
        documentUuid: document_uuid || null,
        metric: safeMetric
      },
      pagination: {
        page: safePage,
    ...
    "Report task by document detail retrieved successfully"
  );
