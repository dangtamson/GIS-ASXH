...
const reportDetailQuerySchema = reportQuerySchema.extend({
  field_uuid: z.string().trim().optional(),
...
function buildFieldTaskScopeQuery(input: {
  workspaceId: string;
  organizationIds: string[];
  fromDate?: string;
  toDate?: string;
  ...
  const fieldFilter =
    fieldList.length > 0 ? sql`and coalesce(d.field_id, t.field_id) in (${buildInList(fieldList)})` : sql``;
  const fieldUuidFilter = fieldUuid ? sql`and coalesce(d.field_id, t.field_id) = ${fieldUuid}` : sql``;
  const docTypeFilter =
  ...
  return sql`
    ...
    filtered_tasks as (
      select distinct
        ...
        t.completed_at,
        t.created_at,
        coalesce(d.field_id, t.field_id) as field_uuid,
        coalesce(field_item.name, 'Chưa xác định') as field_name,
...
export const reportTaskByField = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const assignmentFromFilter = fromDate ? sql`and ta.assigned_at >= ${fromDate}` : sql``;
  const assignmentToFilter = toDate ? sql`and ta.assigned_at <= ${toDate}` : sql``;
  const fieldCatalogFilter =
    fieldList.length > 0 ? sql`and ci.uuid in (${buildInList(fieldList)})` : sql``;
  const fieldTaskFilter =
    fieldList.length > 0 ? sql`and coalesce(d.field_id, t.field_id) in (${buildInList(fieldList)})` : sql``;
  const docTypeFilter =
    docTypeList.length > 0 ? sql`and d.document_type_id in (${buildInList(docTypeList)})` : sql``;
  ...
  const { rows } = await db.execute(sql`
    ...
    field_summary as (
      ...
      where ft.field_uuid is not null
      group by ft.field_uuid
    )
    select
      ci.uuid,
      ci.name,
      coalesce(fs.tong_so_nhiem_vu, 0)::int as tong_so_nhiem_vu,
      coalesce(fs.tong_so_don_vi_thuc_hien, 0)::int as tong_so_don_vi_thuc_hien,
      coalesce(fs.tong_so_don_vi_phoi_hop, 0)::int as tong_so_don_vi_phoi_hop
    from category_items ci
    join categories c on c.uuid = ci.category_id
    left join field_summary fs on fs.uuid = ci.uuid
  ...
  const response = apiResponse.success(
    ...
    "Report task by field retrieved successfully"
  );

  res.status(response.code).send(response);
});

export const reportTaskByFieldDetail = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  const workspaceId = req.workspaceId?.trim();
  if (!workspaceId) {
    const response = apiResponse.error(HttpErrors.MissingParameter("x-workspace-id header"));
    res.status(response.code).send(response);
  ...
  const { fromDate, toDate, organization_ids, field, document_type_id, field_uuid, metric, page, limit } =
    parsed.data;
  const fieldList = parseList(field);
  const docTypeList = parseList(document_type_id);
  const organizationIds = await resolveOrganizationIds(
  ...
  const response = apiResponse.success(
    HttpStatusCode.OK,
    {
      ...
      filters: {
        ...
        field: fieldList.length ? fieldList : null,
        documentTypeId: docTypeList.length ? docTypeList : null,
        fieldUuid: field_uuid || null,
        metric: safeMetric
      }
    },
    "Report task by field detail retrieved successfully"
  );
