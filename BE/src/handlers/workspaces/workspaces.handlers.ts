...
export const createWorkspace = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const result = await db.transaction(async (tx) => {
    ...
    const membership = await createMembership(workspace.uuid, accountId, "admin", true, tx);
...
export const fetchWorkspacesByAccountId = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const result = account.isSuperAdmin
    ? await db.select().from(workspaces).orderBy(workspaces.createdAt).execute()
    : await db
      .select({
  ...
  const response = apiResponse.success(
    ...
    account.isSuperAdmin
      ? `Fetched all workspaces for super admin: ${result.length}`
      : `Fetched member workspaces: ${result.length}`
  );
...
export const updateWorkspaceProfile = asyncHandler(async (req: Request, res: Response): Promise<void> => {
  ...
  const accountIdValidation = validateRequiredUuid(accountId, "Account ID");
  ...
  const validatedAccountId = accountIdValidation.value;
  ...
  const [existingProfile] = await db
    .select()
    .from(profiles)
    .where(and(eq(profiles.accountId, validatedAccountId), eq(profiles.workspaceId, workspaceId)))
    .limit(1);
  ...
  const [updatedProfile] = await db
    ...
    .where(and(eq(profiles.uuid, existingProfile.uuid), eq(profiles.accountId, validatedAccountId)))
    .returning();
