...
import { updateCategoryAdminById, updateCategoryItemAdminById, updateOrganizationAdminById } from "./index.ts";
...
const cases: {
  ...
  {
    name: "categories",
    handler: updateCategoryAdminById,
    payload: { name: "  Category A  ", workspaceId: OTHER_ID },
    expectedSet: { name: "Category A" }
  },
  {
    name: "category items",
    handler: updateCategoryItemAdminById,
    payload: { name: "  Item A  ", createdAt: new Date().toISOString() },
    expectedSet: { name: "Item A" }
  },
  {
    name: "organizations",
    handler: updateOrganizationAdminById,
    payload: { name: "  Org A  ", workspaceId: OTHER_ID },
    expectedSet: { name: "Org A" }
  }
];

describe("master-data update handlers", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
