...
export async function getAccountById(accountId: string): Promise<AccountSelectType[]> {
  const validAccountId = assertValidUuid(accountId, "account ID");

  const equals = eq(accounts.uuid, validAccountId);
  ...
  logger.info(`Retrieved account with UUID: ${validAccountId}`);
