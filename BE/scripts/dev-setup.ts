...
async function findRoleIdByCode(roleCode: "admin" | "user"): Promise<number> {
...
async function createWorkspaceForAccount(options: CreateWorkspaceOptions): Promise<{
  ...
  // Create membership
  const roleId = await findRoleIdByCode(role);
  ...
  logger.info(`✓ Created membership with roleId: ${membership.roleId}`);
