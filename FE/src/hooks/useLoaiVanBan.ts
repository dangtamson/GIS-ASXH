...
export function useLoaiVanBanSelect({
                                     limit = 20,
                                 }: UseLoaiVanBanSelectProps = {}) {

    const [dsLoaiVanBan, setDsLoaiVanBan] = useState<Option[]>([]);
    ...
    const initCategory = async () => {
        try {
            ...
            setDsLoaiDm([]);
            setError(err instanceof ApiError ? err.message : "Không thể tải danh mục loại văn bản.");
        }
    ...
    const fetchData = async (
        ...
        try {
            ...
            const fieldCategoryId = dsLoaiDm
                .filter(
                    (cat) =>
                        normalizeText(cat.code) === "document_type"
                )
            ...
            setHasMore(false);
            setError(err instanceof ApiError ? err.message : "Không thể tải danh sách loại văn bản.");
        } finally {
            setLoading(false);
        }
